...
class InvertedResidual(nn.Module):
    def __init__(
        ...
        self.use_res_connect = (self.stride == 1 and inp == oup)  # 捷径分支使用的条件
...
class MobileNetV2(nn.Module):
    def __init__(
        ...
        # 参数变成8的倍数
        input_channel = _make_divisible(input_channel * width_mult, round_nearest)  # (32, 8)
        ...
        for t, c, n, s in inverted_residual_setting:
            ...
            for i in range(n):
                stride = s if i == 0 else 1  # 子模块第一层的步长从配置文件取, 后面的步长都是1
        ...
        # make it nn.Sequential; 特征提取部分的输出通道数量是1280
